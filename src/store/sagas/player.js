...
export function* next() {
  try {
    ...
    const currentSong = player.list.findIndex(song => song.id === player.currentSong.id);
    const nextSong = player.list[currentSong + 1];
...
export function* previous() {
  try {
    ...
    const currentSong = player.list.findIndex(song => song.id === player.currentSong.id);
    const previousSong = player.list[currentSong - 1];

    if (previousSong) {
      yield put(PlayerActions.setSongRequest(previousSong, player.list));
    }
  } catch (err) {
    console.log(err);
  }
